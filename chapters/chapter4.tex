...
\textbf{AQUI ESTAVA A SECÇÃO DEDICADA AO VIRTUALBENCH - OPINIÃO PROF}
...
A arquitectura do sistema é composta por multiplos ficheiros que comunicam entre si, englobando troca de parâmetros. A parte central é o ficheiro \textit{views.py}, que integra o servidor \textit{Flask} e por ele passa todo o tipo de comunicação. 

Entre as páginas \acrshort{html} e o ficheiro \textit{python}\footnote{Pode-se, neste caso, reduzir a comunicação ao caso mais geral de troca de informação entre ficheiros \acrshort{html} e \textit{python}, já que, no fundo, o ficheiro \textit{views.py} está escrito em \textit{python}.} a comunicação é feita nos dois sentidos - setas amarelas - já que os utilizadores enviam os parâmetros específicos de cada experiência e os resultados são apresentados nas páginas. A Listagem \ref{lst:htmlpy} ilustra a forma como os parâmetros são enviados da página \acrshort{html} para o ficheiro \textit{python} - Linha \ref{line:parametros} - e como os resultados são recebidos - Linha \ref{line:resultado}.
...
O envio de parâmetros para as páginas é feito em \gls{json}, usando a instrução definida na Listagem \ref{lst:returnjson}.
...
\begin{minipage}{0.9\linewidth}
	\begin{lstlisting}[language=python, escapechar=|, caption=Comunicação \textit{python} - \textit{python}, label=lst:pypy]
		...
		configRelays.config_relays_ohm(Resistance, measure_parameter) |\label{line:chamadafuncao}|
	\end{lstlisting}
\end{minipage}

Neste caso, são importados três ficheiros a partir da pasta ``\textit{ctrl\_hardware}'', como descrito na Linha \ref{line:ficheiros} e a função \textit{config\_relays\_ohm}, que está definida no ficheiro \textit{configRelays}, é chamada com os parâmetros \textit{Resistance} e \textit{measure\_parameter}, Linha \ref{line:chamadafuncao}.

O controlo do \acrshort{virtualbench}, representado na Figura \ref{fig:diagramasimplificado} pelas setas a cor vermelha, é feito com recurso à libraria \textit{pyVirtualbench}.
...
No exemplo seguinte, o resultado da execução das instruções na Linha \ref{line:virtualbench} e Linha \ref{line:powersource}, referente à Listagem \ref{lst:exemplops} é:
\begin{itemize}
...
No fim da execução do ficheiro, o \acrshort{virtualbench} e os restantes instrumentos têm de ser libertados, tal como se pode ver na Linha \ref{line:virtualbenchlibertado} e Linha \ref{line:pslibertada}, da Listagem \ref{lst:exemplops}.
...
No \acrshort{lare}, o controlo dos relés é feito com recurso ao \textit{RaspberryPI5}, representado na Figura \ref{fig:diagramasimplificado} pela seta azul. A ligação ao \acrshort{lare} é feita por um cabo IDC, 40 pinos, tal como descrito no \textit{datasheet} - ver como fazer a referênia.

Como já foi referido na Secção \ref{sec:arquitectura}, a comunicação entre o \textit{RaspberryPI5} e o servidor é feita através de um \textit{socket}. No caso geral, é enviada uma \textit{string} correspondente ao número de relés que se pretende controlar.

Pela consulta dos esquemas no \textit{datasheet}, a Lei de \textit{Ohm} é composta por 8 relés e as restantes experiências por 13 relés. Logo, será enviada uma \textit{string} com 8 ou 13 \textit{bits}, consoante a experiência que se pretende controlar. 
