...
Como já foi referido na Secção \ref{sec:experiencias}, a implementação dos filtros foi feita após a conclusão dos rectificadores. De forma a integrar estas experiências no circuito já implementado, foi necessário realizar um \textit{bypass} ao díodo $D_{1}$ (indicado pelo retângulo tracejado vermelho na Figura \ref{fig:rectificacao_filtragem_full}). Isto permitiu a inclusão de uma resistência ou de um condensador.
% dependendo da versão do filtro, conforme representado no rectângulo tracejado magenta.
A Figura \ref{fig:filtro_pa}
