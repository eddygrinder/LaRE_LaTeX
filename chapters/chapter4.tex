...
Sendo assim, a tensão de saída calculou-se com base na expressão dada no mesmo documento, representada na Equação \ref{eq:calculolm317}:

\begin{equation} \label{eq:calculolm317}
	V_{O} = V_{REF} (1 + \frac{R_{2}}{R_{1}}) + (I_{ADJ} \times R_{2})
\end{equation}

Resolvendo a equação de forma a determinar os valores das resistências $R_{1}$ e $R_{2}$, para uma tensão de \SI{5}{\volt}, obtiveram-se os valores de \SI{240}{\ohm} e \SI{1}{\kilo\ohm}, respectivamente. A corrente de saída é limitada a \SI{1}{\ampere}.
