...
A comunicação com o \textit{RaspberryPI} é feita através de \textit{sockets}, método que já foi descrito na Secção~\ref{sec:arquitectura}. Depois de testados os parâmetros no ficheiro \textit{configRelays.py}, é enviada uma \textit{string} - via rede com ou sem fios - correspondente ao número de relés a activar: 8\textit{bits} ou 13\textit{bits} (consoante os circuitos) \textbf{Especificar? OPINIÃO PROF}.
