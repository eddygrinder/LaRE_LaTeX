...
A arquitectura do sistema é composta por multiplos ficheiros que comunicam entre si, englobando troca de parâmetros. Pelo ficheiro \textit{views.py}, que integra o servidor \textit{Flask}, passa somente a comunicação estritamente necessária. \textbf{Já foi referenciado atrás que pelo views passa, seomente, a comunicação estritamente necessária e que se tentou manter este ficheiro o mais simples possível}  com excepção do controlo do \acrshort{virtualbench} que é feito pela libraria \textit{pyVirtualBench}. 
...
%usando jsonify({'measurement_result': measurement_result}), que converte e devolve os dados numa API Flask.
...
Neste caso, são importados três ficheiros

Através do diagrama é possível perceber que há três  comunicação se baseia na troca de parâmetros qualquer troca de parâmetros passa pelo ficheiro \textit{views.py} - Servidor \textit{Flask}. As setas coloridas representam as diferentes trocas entre os diversos tipos de ficheiros.
\begin{itemize}
	\item \acrshort{html} e \textit{python};
	\item \textit{python} e \textit{python}.
\end{itemize}
há três tipos de trocas comunicação entre o \textit{Flask} e o \textit{pyVirtualBench} é feita através de \acrshort{http} e \acrshort{url}s. O utilizador, ao aceder à página \textit{ohm.html}, pode escolher os valores de tensão e resistência, que são enviados para o servidor \textit{Flask} através de \acrshort{http} e \acrshort{url}s. O servidor \textit{Flask} recebe os parâmetros e envia-os para o \textit{pyVirtualBench}, que executa a experiência. Os resultados são enviados de volta para o servidor \textit{Flask}, que os apresenta ao utilizador.
