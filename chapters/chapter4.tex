...
Como se optou pela liberdade de escolha aos utilizadores, a abordagem à implementação foi um desafio. A melhor forma de descrever esta experiência é dividi-la por partes. A Figura \ref{fig:fluxohm} representa o fluxograma do processo de execução da experiência.
\begin{figure}[hbtp]
	\centering
...
De uma forma geral, e como se apresenta na Listagem \ref{lst:exemploenvioparametros}, a comunicação entre a página \textit{ohm.html} e o \textit{script views.py} é feita da seguinte forma:
\begin{center}
	\begin{minipage}{0.7\linewidth}
...
Como exemplo, os selectores dos parâmetros ``$V_{CC}$'' e ``R'' estão inicialmente desabilitados. Seleccionando ``OK'' as opções ficam disponíveis, como se pode ver na Figura \ref{fig:pagohm}. Neste caso, o parâmetro é enviado para o \textit{script} \textit{views.py}, tal como representado na Listagem \ref{lst:envioparametros}.

\begin{minipage}{0.9\linewidth}
	\begin{lstlisting}[language=Python, caption=Envio de parâmetros para \textit{views.py}, label=lst:envioparametros]
...
Do lado do \textit{script} \textit{views.py}, o parâmetro é recebido e, consoante o valor, são executadas as funções necessárias, tal como pode ser visto na Listagem \ref{lst:recepcaoparametros}.
...
Pela Listagem \ref{lst:recepcaoparametros} pode ver-se que, quando é recebido o parâmetro ``OK'', é chamada a função \textit{OK()} do \textit{script} \textit{configVB.py}. O procedimento é em tudo idêntico quando é seleccionado o parâmetro ``STOP''.

O envio dos restantes parâmetros, $V_{CC}$, R, medição de corrente (I) ou tensão (U) e a construção do gráfico é feito de forma idêntica.
