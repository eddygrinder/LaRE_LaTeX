...
No entanto, no fim da execução do ficheiro, o \acrshort{virtualbench} e os restantes instrumentos têm de ser libertados, Listagem \ref{lst:libertarinstrumentos}:
\begin{center}
	\begin{minipage}{0.75\linewidth}
...
\textbf{Talvez precise de melhor explicação - ou como exemplo quando se falar dos ficheiros} Sempre que um ficheiro ou \textit{script} é chamado é criado um objecto diferente para cada instrumento. Inclusive, havendo várias funções no mesmo ficheiro, os objectos criados têm de ser passados entre funções ou então, terminar e chamar o(s) instrumento(s) entre chamadas de funções. Outra hipótese seria o uso de variáveis globais. 
...
\vspace{1cm}
\hrule
\textbf{Colei o que tinha à frente - minha referência}
\hrule
\vspace{1cm}

\vspace{1cm}
\hrule
\textbf{PENSO QUE ATÉ AQUI PODEM FICAR ESTAS SECÇÕES}
\hrule
\vspace{1cm}
...
\begin{itemize}
	\item \textit{views.py};
	\item \textit{configRelays.py}.
\end{itemize}

O ficheiro \textit{views.py} é parte central, pois nele estão definidas todas as rotas \textit{flask}. Quer isto dizer que, qualquer troca de dados ou argumentos entre ficheiros e/ou \textit{scripts}, passa obrigatoriamente por este ficheiro. Sendo assim, tentou-se, sempre que possível, reduzir o código ao estritamente necessário.

Além do ficheiro \textit{views.py}, foram criados os seguintes ficheiros específicos para cada experiência:
\begin{itemize}
	\item configVB.py - controla a experiência da Lei de Ohm;
	\item mixed\_signal\_oscilloscope.py - controla as experiência dos rectificadores/filtros;
	\item store\_ps\_dmm.py - organiza e mantém o estado de variáveis globais para diferentes medições (tensão, corrente) e equipamentos (ps, dmm, virtualbench, mso) de forma centralizada.
	\item configRelays.py - controla os relés \textit{via} envio da trama de bits para o PI5.
\end{itemize}

FALAR DA PARTE HTML PARA DEPOIS EMBARCAR NA TROCA DE DADOS
...
A melhor forma de descrever esta experiência é dividi-la por partes - \textbf{duas partes, como é referido no HW - lei de ohm e os rectificadoers/filtros}. A Figura \ref{fig:fluxohm} representa o fluxograma do processo de execução da experiência.
\begin{figure}[hbtp]
	\centering
