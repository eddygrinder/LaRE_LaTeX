...
A configuração inicial do \acrshort{virtualbench} é necessária, uma vez que, antes de se iniciar o processo de medições, optou-se por activar a fonte de alimentação, alimentar os relés e a fonte de alimentação composta pelo LM317 - \textbf{NOTA: Fazer referência à secção de HW}. Desta forma, implementou-se nas páginas das experiências, um botão de ``OK'' - que habilita os selectores e configura a fonte do \acrshort{virtualbench} e ``STOP/RESET'' - que desabilita os selectores e desactiva as fontes enquanto a experiência está inactiva.

No entanto, para a primeira etapa, o foco será a cofiguração do na comunicação entre a página \textit{ohm.html} e o ficheiro \textit{configVB.py}, representado na Figura \ref{fig:comohmconfigvb}.
