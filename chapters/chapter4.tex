...
A comunicação com o \textit{Raspberry Pi} é realizada por meio de \textit{sockets}, um método já descrito na Secção~\ref{sec:arquitectura}. Após a configuração e teste dos parâmetros \sout{no ficheiro \textit{configRelays.py}}, uma \textit{string} correspondente ao número de relés a activar - 8 \textit{bits} ou 13 \textit{bits}, dependendo dos circuitos - é enviada através de uma \textit{interface} de rede. No ficheiro \textit{configRelays.py} está implementado a forma como a comunicação é realizada.
