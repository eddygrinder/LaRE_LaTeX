...
\textbf{NOTA: Acho que os par+agrafos seguintes ficariam melhores nas respectivas secções}
...
\label{sec:lei_ohm}
O funcionamento do circuito é relativamente simples e está representado na Figura \ref{fig:esq_geral_ohm}. Qualquer que seja a resistência a medir, é fechado o respectivo relé - $K_{1}$ ou $K_{2}$ ou $K_{3}$; a fonte $U_{CC}$ é ligada ao circuito através do relé $K_{4}$ e $K_{7}$.
...
\begin{enumerate}
	\item Ligação ao \gls{RaspberryPI} através de um conector \acrfull{idc} de 40 pinos;
	\item Relés;
	\item Ligações aos aparelhos de medida;
	\item \textit{Drivers} de relés e registo de deslocamento;
	\item Resistências em estudo;
	\item Ligação à placa seguinte através de conectores ``Arduino stackable''.
\end{enumerate}

\subsubsection{Rectificadores}
\label{sec:rectificadoresfiltros}
Na implementação dos rectificadores houve a necessidade de utilizar dois tipos de tensão de entrada. Como referido na Secção \ref{sec:fontesalimentacao}, o circuito de meia onda utiliza o gerador de sinal mas devido ao problema de massa, já explicado na mesma secção, foi necessário usar um transformador.
