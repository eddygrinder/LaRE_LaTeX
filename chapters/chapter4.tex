...
No entanto, o caso das medições da Lei de \textit{Ohm}, reveste-se de alguma particularidade. Para garantir a persistência das medições ao longo de várias chamadas à API, sem necessidade de utilizar uma base de dados ou ficheiros externos, foi implementado um módulo Python (estado\_medicoes.py) que recorre a variáveis globais para armazenar os dados.

Como o servidor Flask permanece ativo durante o processo de medição, estas variáveis mantêm-se em memória entre chamadas, permitindo o armazenamento cumulativo de valores de corrente e tensão. Esta abordagem simplifica a gestão de estado numa fase inicial do desenvolvimento, sendo eficiente desde que o processo não seja reiniciado.
...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Filtro passa-baixo\label{fig:meiaondamenuCTRL}]{{\includegraphics[width=6cm]{figures/meiaonda_page_controlo-FULL.png} }}%
		\qquad
...
\label{sec:filtrosSW}
As páginas dos filtros passa-baixo e passa-alto, são em tudo idênticas à do rectificador de meia onda.
...
Eventualmente, poder-se-ia ter adicionado mais resistências/condensadores aos filtros, nomeadamente, no rectângulo tracejado vermelho, de forma a permitir uma maior versatilidade no estudo. No entanto, além de não existir o número suficiente de relés em contexto laboratorial, considerou-se que, desta forma, o estudo e os objectivos da experiência, ficariam provados.

O intervalo de frequência mantém-se entre os \SI{50}{\hertz} e \SI{2000}{\hertz}. A Figura \ref{fig:ondacompletamenuCTRL} apresenta a página do filtro passa-baixo, sendo que a única diferença para o passa-alto é o esquema representado na Figura \ref{fig:ondacompletamenuCTRL}.