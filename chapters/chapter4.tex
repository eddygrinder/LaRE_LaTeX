...
\textbf{Eventualmente os rectificadores podem ficar na mesma secção}

Nestas duas experiências pretende-se estudar e avaliar a diferença nos dois tipos de rectificação e a influência que têm na variação da tensão de \textit{ripple}.

O estudo tem como base o circuito simplificado representado na Figura \ref{fig:meiasimplificado}, sendo que o circuito completo está representado \textbf{REFERÊNCIA AO DATASHEET}.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/MeiaOndaSimplificado.png}
	\caption{Esquema simplificado - Recitificador de Meia-Onda}
	\label{fig:meiasimplificado}
\end{figure}

A versatilidade desta experiência permite, não só, estudar a rectificação de meia-onda, mas também a variação da tensão de \textit{ripple}. O tutor ou utilizador controla a frequência do sinal de entrada, representado no esquema da Figura \ref{fig:meiasimplificado} por $V_{G+}$ e $V_{G-}$ (rectângulo tracejado a azul) e mediante a escolha do par condensador/resistência, avalia a influência que tem na variação da tensão de \textit{ripple}.
