...
Portanto, como se optou pela liberdade de escolha aos utilizadores, a abordagem à implementação foi um desafio. A melhor forma de descrever esta experiência é dividi-la por partes.

\textbf{Ficheiros associados à experiência}: Na implementação desta experiência foram criados os seguintes ficheiros:
\begin{itemize}
...
A Figura \ref{fig:fluxohm} representa o fluxograma do processo de execução da experiência.
\begin{figure}[hbtp]
	\centering
...
A página que é apresentada ao utilizador está representada na Figura \ref{fig:introohm}.
\begin{figure}[hbtp]
	\centering
...
Os selectores dos parâmetros ``$V_{CC}$'' e ``R'' estão inicialmente desabilitados. Seleccionando ``OK'' as opções ficam disponíveis, como se pode ver na Figura \ref{fig:introohmhabilitado}. Neste caso, o parâmetro é enviado para o \textit{script} \textit{views.py}, tal como pode representado na Listagem \ref{lst:envioparametros}.

\begin{minipage}{1\linewidth}
	\begin{lstlisting}[language=Html, caption=Envio de parâmetros para \textit{views.py}, label=lst:envioparametros]
...
Do lado do \textit{script} \textit{views.py}, o parâmetro é recebido e, consoante o valor, são executadas as funções necessárias, tal como pode ser visto na Listagem \ref{lst:recebeparametros}.
