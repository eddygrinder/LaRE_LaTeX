...
Observa-se, através da Figura \ref{fig:sedraripple} e também da Equação \ref{eq:vripple} que, quando a constante de tempo $CR >> T$, a tensão de \textit{ripple} é pequena. Sendo que, $v_{O}$, ou seja, a tensão à saída do filtro é practicamente constante e dada pela Equação \ref{eq:tensaosaida}:

\begin{equation} \label{eq:tensaosaida}
	v_{O} = V_{P} - \dfrac{1}{2}V_{r}	
\end{equation}

No entanto, pode-se tornar este circuito mais eficiente se se usar a rectificação de onda completa e como pode ser visto na Figura \ref{fig:sedraripplecompleta}, a frequência do \textit{ripple} é o dobro da frequência da onda de entrada.
...
--- se calhar nem explicava os blocos ---

O Bloco 1 difere nas duas experiências por que, tal como foi referido na Secção \ref{sec:fontealternada}, o rectificador de meia onda é alimentado pelo gerador de sinal e o rectificador de onda completa pelo transformador monofásico - rectângulo tracejado a (...). O Bloco 4 não está montado/projectado, visto que o objectivo é o estudo da variação da tensão de \textit{ripple} com a frequência. \textbf{verificar se com a FREQUÊNCIA o rectificador de meia onda, como etá alimentado pelo gerador de sinal, permite acaliar como varia o ripple com a frequência. Já o rectificador de onda completa trabalha `frequencia da rede, 60 hz'}.
