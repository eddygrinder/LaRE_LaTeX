...
Tendo em conta que os rectificadores já se encontravam projectados e testados, considerou-se vantajoso integrar no \acrshort{lare} duas experiências complementares, destinadas ao estudo práctico de filtros passa-alto e passa-baixo. As modificações feitas foram relativamente simples, uma vez que os blocos de rectificação e filtragem são comuns às duas experiências. Na Figura \ref{fig:filtrosesqgeral} estão representados os esquemas simplificados dos filtros passa-baixo e passa-alto.

\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Filtro passa-baixo]{{\includegraphics[width=6cm]{figures/filtro_PB_simplificado.png} }}%
		\qquad
		\subfloat[\centering Filtro passa-alto]{{\includegraphics[width=6cm]{figures/filtro_PA_simplificado.png} }}%
...
Para que estas experiências pudessem ser integradas no circuito, houve a necessidade de se fazer um \textit{bypass} ao díodo $D_{1}$ - rectângulo tracejado vermelho, Figura \ref{fig:rectificadoresimplificado}. 
Desta forma, conseguiu-se colocar as resistências e condensadores do rectificador, em série com a resistência e condensador do filtro.
O esquema simplificado dos filtros, comandados por dois relés simples, $K_{10}$ e $K_{11}$, está representado na Figura \ref{fig:filtragemsimplificado}.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.5\textwidth]{figures/filtros_simplificado.png}
	\caption{Esquema dos filtros simplificado}
	\label{fig:filtragemsimplificado}
\end{figure}
...
=======
Para que estas experiências pudessem ser integradas no circuito, houve a necessidade de se fazer um \textit{bypass} ao díodo $D_{1}$. Desta forma, conseguiu-se colocar as resistências e condensadores do rectificador, em série com a resistência e condensador do filtro.
>>>>>>> 8eaf0f68fdd84ecc7d3f67f38abff8c847233568

Explicar agora o funcionamento
