...
Os restantes instrumentos usados no \acrshort{lare}	seguem a mesma lógica de funcionamento, como referido em \ref{item:logica} - \textbf{NOTA: Ver esta referência}.

sendo que, para cada um deles, são definidos os parâmetros específicos e realizadas as configurações necessárias para a experiência em questão.


Para medições etc, etc, o conjunto completo de funções e configuraçõoes que nos permite controlar e configurar o V encontra-se aqui - . No contexto deste projecto, vamos utilizar a biblioteca, como já foi referido e suportarmo-nos dos exemplos dados na biblioteca instalada a aprtir do respositório do github. Os exemplos dou só alguns, não vale a pensa fá-los todos. Voltámos ao exemplo da fonte de alimentação e podesmo pegar no exemplo do dmm para apresentar como se faz as medições da tensão e corrente e respectivas configurações
...
%Partindo da sequência anterior, o primerio passo (1.) é a habilitação dos selectores e a configuração da fonte de {25}{\volt}. Pela análise da Figura \ref{fig:fluxohm}, os parâmetros são enviados da página \textit{ohm.html} para o \textit{script views.py}, da forma como indica a Listagem \ref{lst:exemploenvioparametros}.
