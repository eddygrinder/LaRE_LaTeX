...
\subsection{Páginas \textit{Web} - esta secção passou para a parte das experiências (Lei de Ohm)}
\label{sec:paginas}
...
Já foi referido na secção anterior - REF - que não é objetivo deste estudo dissecar todas as linhas de código ou ficheiros do projeto. Optou-se, assim, por apresentar um estudo que permita, de modo geral, compreender a base da Arquitetura de Comunicação, a troca de parâmetros e as configurações entre os diversos tipos de ficheiros.

Nesta secção, abordam-se INTERFACE WEB de forma mais específica e detalhada os aspectos particulares das experiências. Considerou-se, assim, que a melhor abordagem para estudá-las é através de um exemplo prático e concreto.

Comum a todas as experiências é a configuração inicial do \acrshort{virtualbench}. Esta configuração é necessária, uma vez que, antes de se iniciar o processo de medições, optou-se por activar a fonte de alimentação que alimenta os relés e os \textit{drivers} de relés e a fonte de alimentação composta pelo LM317 que alimenta os registos de deslocamento - \textbf{NOTA: Fazer referência à secção de HW}. Desta forma, implementou-se nas páginas das experiências, um botão de ``OK'' - que habilita os selectores e configura a fonte do \acrshort{virtualbench} e ``STOP/RESET'' - que desabilita os selectores e desactiva as fontes enquanto a experiência está inactiva.

\subsubsection{\sout{Lei de Ohm - Páginas web} Interface web}
\label{sec:paginas}

SE CALHAR NO PONTO 4.2.4 TROCAVA O TÍTULO EXPERIÊNCIAS POR INTERFACE WEB E DENTRO DESTA SECÇÃO DIVIDIA POR EXPERIÊNCIAS
...
O menu de separadores verticais foi retirado do \textit{site} \href{https://www.w3schools.com/howto/howto_js_vertical_tabs.asp}{\textit{w3schools}} e modificado de acordo com as necessidades do projecto. \sout{tal como pode ser visto, por exemplo, na Figura \ref{fig:pagmenu} que apresenta a página inicial.}
...
Depois de seleccionada a experiência, as páginas seguem a mesma estrutura de menus e o mesmo padrão de organização e navegação:
\begin{itemize}
	\item ``Introdução'' -  onde é apresentado o esquema e uma breve introdução à experiência a realizar;
	\item ``Controlo'' - onde o utilizador pode escolher os parâmetros da experiência e as medições a efectuar.
\end{itemize}

Como exemplo, as Figuras \ref{fig:ohm_intro} e \ref{fig:ohm_ctrl} apresentam, respetivamente, as páginas de Introdução e Controlo da experiência sobre a Lei de \textit{Ohm}.
...
A verificação e detecção dos erros acontece quando o utilizador tenta seleccionar um dos parâmetros antes de estes estarem habilitados. O alerta de erro é implementado nas páginas \acrshort{html} das experiências, usando o método \textit{alert()} do \textit{JavaScript}.
