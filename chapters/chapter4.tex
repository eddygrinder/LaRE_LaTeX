...
\begin{figure}[hbtp]
	\centering
	\includegraphics[width=1\textwidth]{figures/ohm_diagrama_ALT.png}
...
A verificação dos erros e escolha de parâmetros é feita na página \textit{ohm.html}, tal como se pode ver pela Listagem \ref{lst:erro} e pelo exemplo da Figura \ref{fig:erropagina}.
...
De referir que o botão ``OK'', além de habilitar os selectores, activa e configura, previamente, a fonte de tensão de \SI{25}{\volt}, como descrito na Secção \ref{sec:fontesalimentacao}. Optou-se por esta abordagem, uma vez que, quando se efectuarem as medições, os relés e circuitos integrados já estarão alimentados. Em termos de \textit{software} e da própria estrutura de programação, achou-se vantajoso o envio de parâmetros e configuração do \acrshort{virtualbench} ser separado do envio dos restantes parâmetros de medição,
