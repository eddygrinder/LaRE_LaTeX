...
\subsection{Comunicação-Protocolo ou o raio que te parta}
\label{sec:comunicacao}
Nesta secção, o foco será analisar a comunicação e troca de parâmetros entre os diversos tipos de ficheiros e o servidor \textit{Flask}, nomeadamente, o ficheiro \textit{views.py}. Na Figura \ref{fig:fluxohm} é apresentado um diagrama que ilustra o funcionamento geral da experiência da Lei de \textit{Ohm}.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.9\textwidth]{figures/ohm_diagrama.drawio.png}
	\caption{Diagrama da experiência Lei de \textit{Ohm} [REVER]}
	\label{fig:fluxohm}
\end{figure}

Através do diagrama, é possível perceber que a comunicação dá-se entre dois tipos de ficheiros:
\begin{itemize}
