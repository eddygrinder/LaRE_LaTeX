...
As experiências foram implementadas de forma independente. Isto permitiu um melhor desenvolvimento, foco, organização, detecção e correcção de erros. Após os testes e conclusão do circuito da Lei de \textit{Ohm} passou-se ao projecto dos rectificadores. O desenvolvimento inicial concentrou-se no estudo dos rectificadores. No entanto, após o desenvolvimento e alguns testes, e uma vez que estas experiências partilham alguns elementos/componentes/blocos, \sout{nomeadamente, o filtro incluido no rectângulo laranja tracejado na Figura \ref{fig:rectificacao_filtragem_full},} considerou-se vantajoso adicionar ao \acrshort{lare} esta experiência complementar, voltada para o estudo práctico de filtros passa-alto e passa-baixo.

As experiências foram implementadas e desenvolvidas de forma independente, uma abordagem que favoreceu um maior foco, organização e eficiência na identificação e correção de erros. Após a conclusão dos testes e do circuito da Lei de \textit{Ohm}, iniciou-se o desenvolvimento dos rectificadores. Embora a abordagem inicial se tenha concentrado exclusivamente nos rectificadores, estas experiências têm alguns elementos e componentes em comum. Diante disso, decidiu-se incluir no \acrshort{lare} uma experiência complementar dedicada ao estudo prático de filtros passa-alto e passa-baixo, ampliando o alcance e a aplicabilidade do projeto.

Na organização e no desenvolvimento do \textit{hardware} e \textit{software} dos circuitos que compõem as experiências, decidiu-se estruturar o projeto em duas partes distintas: Lei de \textit{Ohm} e Retificadores/Filtros. Esta divisão foi adoptada devido ao facto de que entre os rectpara facilitar o desenvolvimento e a implementação dos circuitos, permitindo um foco específico em cada área, de forma a otimizar a integração entre o \textit{hardware} e o \textit{software} e garantir que cada experiência seja explorada de maneira mais aprofundada e didática.

\textbf{Vou considerar que cada experiência compreende dois circuitos - É NECESSÁRIO VERIFICAR A COÊRENCIA ATRÁS}
...
\sout{Os esquemas finais, simplificados, estão representados nas Figuras e Figuras}
...
\textbf{Eventualmente as Figuras 4.17 e 4.18 podem ser retiradas}
...
\sout{Estas duas experiências foram integradas na mesma placa. }

Na Figura \ref{fig:rectificacao_filtragem_full} está representado o esquema completo e simplificado das duas experiências, sendo que:
\begin{itemize}
	\item Meia onda: rectângulo tracejado vermelho e laranja;
	\item Onda completa: rectângulo tracejado verde e laranja;
	\item Filtros passa-alto e passa-baixo: rectângulo tracejado magenta e laranja.
\end{itemize}
...
No estudo dos rectificadores, meia onda e onda completa, pretende-se estudar e avaliar a diferença entre os dois tipos de rectificação e a influência que têm na variação da tensão de \textit{ripple}. 
...
A implementação destas duas experiências foi feita de forma a que se possa estudar e avaliar o \textit{ripple} dos rectificadores, consoante as quatro combinações possíveis dos pares resistência/condensador. No caso da rectificação de meia onda, é ainda possível variar a frequência entre os \SI{50}{\hertz} e \SI{2000}{\hertz} mas no caso da rectificação de onda completa, a frequência é fixa ao valor da rede eléctrica - \SI{60}{\hertz}. Se se pretender estudar qualquer uma das experiências, os relés seriam activados da forma representada na Tabela \ref{Table:exemplomeiaonda}. Neste exemplo, o filtro do rectificador de meia onda é composto por $R_{1}/C_{1}$ e o filtro do rectificador de onda completa é composto por $R_{2}/C_{2}$
...
\subsubsection{Filtros Passa-Baixo e Passa-alto}
\label{sec:filtros}
...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Filtro passa-baixo\label{fig:filtro_pb}]{{\includegraphics[width=6cm]{figures/filtro_PB_simplificado_trac.png} }}%
		\qquad
		\subfloat[\centering Filtro passa-alto\label{fig:filtro_pa}]{{\includegraphics[width=6cm]{figures/filtro_PA_simplificado_trac.png} }}%
...
Para integrar estas experiências no circuito, foi necessário realizar um \textit{bypass} ao díodo $D_{1}$ (indicado pelo retângulo tracejado vermelho na Figura \ref{fig:rectificadoresimplificado}). Isto permitiu a inclusão de uma resistência ou de um condensador, dependendo da versão do filtro, conforme representado na Figura~\ref{fig:filtro_pb} e Figura~\ref{fig:filtro_pa}, dentro do retângulo tracejado azul.

O esquema final simplificado, que inclui, então, o estudo da rectificação de meia onda e dos filtros passa-alto e passa-baixo, está representado na Figura \ref{fig:rectificacao_filtragem_simplificado}.

A escolha das experiências e combinação das resistências e condensadores é controlada pelo utilizador/\textit{software}, tal como descrito na Secção (...). A Tabela \ref{Table:rectificadoresfiltros} apresenta o estado dos relés, $K_{1}$ a $K_{9}$ consoante a experiência que se pretende realizar.
...
\begin{table}[htb]
	...
	\begin{tabular}{lccccccccc}
		...
		Onda Completa  & 0 		 & 1       & 0       & 0       & 1       & 0       & 0       & 1  	   & 1    \\
		\midrule
		Filtro Passa-Alto      & 1		 & 0       & 1       & 1       & 0       & 1       & 1       & 0 	   & 0      \\
		\midrule
		Filtro Passa-Baixo      & 1		 & 0       & 1       & 1       & 0       & 1       & 1       & 0 	   & 0      \\
		\bottomrule
	\end{tabular}
\end{table}
