...
Sendo assim, o ficheiro \textit{receive.py} é executado inicialmente e aguarda pela envio da \textit{string}. De referir que, embora o \textit{socket} esteja configurado em \textit{blocking mode} - quer isto dizer que o cliente ficará bloqueado na Linha~\ref{line:blockmode}, da Listagem \ref{lst:socketblock}, esperando uma resposta do servidor \cite{Sockets} (acontece quando a string for totalmente enviada) - optou-se por enviar uma confirmação do servidor ao cliente. Não sendo esta confirmação estritamente necessária, foi adicionada como auxílio à depuração.
