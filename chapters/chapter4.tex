...
\begin{itemize}
	...
	\item Medição da corrente:
	      \begin{itemize}
		      \item Os relés $K_{5}$ e $K_{8}$ são abertos e $K_{6}$ é fechado. Desta forma, o amperímetro fica em série com a resistência a medir - entre os pontos B e A.
	      \end{itemize}
\end{itemize}

Como se pode pode ver na Figura \ref{paineldianteiro}, o multímetro digital 

Por exemplo, se se pretendesse estudar a Lei de Ohm para a resistência $R_{1}$, os relés seriam activados da forma representada na Tabela \ref{Table:exemplomedicaoohm}:
...
De referir que, se se expandir o \acrshort{lare} com mais circuitos é possível utilizar o voltímetro e o amperímetro da forma como estão configurados. Tal como pode ser visto na Figura \ref{fig:esq_geral_ohm}, o voltímetro pode está disponível entre os pontos A e C e o amperímetro entre os pontos B e C.

da forma como está implementado o voltímetro e o amperímetro, é possível utilizá-los noutros circuitos, se se expandir o \acrshort{lare} com mais experiências. De facto, desactivando  isolá-los completamente do cira medição é feita em simultâneo. No entanto, a implementação pode ser alterada para que a medição seja feita de forma independente.
