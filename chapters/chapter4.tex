...
\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/sedra_ripple.png}
...
Observa-se, através da Figura \ref{fig:sedraripple} e também da Equação \ref{eq:vripple} que, quando a constante de tempo $CR >> T$, a tensão de \textit{ripple} é pequena. Sendo que, $v_{O}$, ou seja, a tensão à saída do filtro é practicamente constante e dada por, $v_{O} = V_{P} - \dfrac{1}{2}V_{r}$.

A Figura \ref{fig:sedraripple} representa a forma de onda à saída do Bloco de Filtragem, no que diz respeito a um rectificador de meia onda. No entanto, pode-se tornar este circuito mais eficiente se se usar a rectificação de onda completa. Como pode ser visto na Figura \ref{fig:sedraripplecompleta}, a tensão de \textit{ripple} é menor, já que a frequência de onda completa é o dobro da frequência da onda de meia onda.

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/sedra_ripple_OC.png}
