...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Ligação SN74HC595 - ULN2003A\label{fig:comandorelesfull}]{{\includegraphics[width=6cm]{figures/comandoreles_FULL.png} }}%
		\qquad
...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Esquemático\label{fig:esqlm317}]{{\includegraphics[width=6.3cm]{figures/sch_fonte5V.png} }}%
		\qquad
...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Esquema implementado\label{fig:esq_geral_ohm}]{{\includegraphics[width=6.3cm]{figures/esquema_simplificado_OHM.png} }}%
		\qquad
		\subfloat[\centering Esquema com os relés duplos\label{fig:full_reles}]{{\includegraphics[width=6.3cm]{figures/esquema_simplificado_OHM_IDEAL.png} }}%
...
No Flask utiliza-se o \textit{decorator} \textit{\textbf{route()}} para associar uma função a um \acrshort{url}, tal como pode ser visto na Listagem \ref{lst:decoradorroute}.
...
\subsubsection{\textit{Blueprints}}
...
Isto quer dizer que, ao registar a \textit{blueprint} com o prefixo ``/'', os \acrshort{url} serão acessíveis através da raiz da aplicação.
...
No contexto da documentação do Flask e implementação de uma base de dados, são apresentadas duas alternativas: SQLite 3 e SQLAlchemy. Tal como sugerido na documentação, usou-se a extensão Flask-SQLAlchemy, descrita no ficheiro \textit{\textunderscore\textunderscore init.py\textunderscore\textunderscore}.
...
\begin{itemize}
	...
	\item \textins{ohm, meiaonda, ondacompleta, passaalto, passabaixo}.html - Visualização.
\end{itemize}
...
No entanto, o processo de medição na experiência da Lei de Ohm possui características específicas que justificam uma explicação detalhada. A Figura~\ref{fig:processoleituraUI} ilustra o procedimento de leitura das medições de tensão e corrente.
...
\subsubsection{Comunicação com Raspberry PI}
...
Através da análise dos exemplos disponibilizados para o \acrshort{virtualbench} (e que estão definidos no repositório já mencionado do GitHub), foi possível compreender a estrutura geral de configuração aplicada aos instrumentos e fontes de alimentação. Com base nessa análise, determinou-se a seguinte lógica de funcionamento:
...
Comum a todas as experiências é a configuração inicial do \acrshort{virtualbench}. Esta configuração é necessária, uma vez que, antes de se iniciar o processo de medições, optou-se por activar a fonte de alimentação que alimenta os relés e os \textit{drivers} e a fonte de alimentação composta pelo LM317 que alimenta os registos de deslocamento, tal como descrito na subsecção \ref{sec:fontesalimentacao}. Desta forma, implementou-se nas páginas das experiências, um botão de ``OK'' - que habilita os selectores e configura a fonte do \acrshort{virtualbench} e ``STOP/RESET'' - que desabilita os selectores e desactiva as fontes enquanto a experiência está inactiva. 

A verificação e detecção dos erros, Figura \ref{fig:erropagina}, acontece quando o utilizador tenta seleccionar um dos parâmetros antes de estes estarem habilitados ou, no caso dos rectificadores e filtros, colocar o valor da frequência fora do valor definido nas instruções. O alerta de erro é implementado nas páginas \acrshort{html} das experiências, usando o método \textit{alert()} do JavaScript.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Página de controlo e realização da experiência.
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{itemize}
...
% No entanto, no que diz respeito ao controlo das experiências, tal como referido na Secção \ref{sec:interfaceweb}, a experiência da Lei de Ohm apresenta um maior grau de complexidade, dado que há um número maior de parâmetros e o utilizador ou aluno tem um controlo mais amplo sobre a experiência.
...
\begin{figure}[hbtp]
	\centering%
		...
		\caption{\textins{Exemplo} Experiência Lei de Ohm}%
		\label{fig:pagohm}%
	\end{figure}
...
Nestas duas experiências, pretende-se estudar e avaliar a diferença entre estes dois tipos de rectificadores, tanto ao nível da rectificação como da filtragem, nomeadamente no estudo da variação da tensão de \textit{ripple}. A principal diferença, como foi referido na subsecção \ref{sec:rectificadoresfiltros}, reside no valor da frequência. Enquanto que no rectificador de meia onda, o sinal de entrada é alimentado pelo gerador de sinal do \textit{virtualbench}, o rectificador de onda completa, devido ao problema de massa já referenciado na subsecção \ref{sec:fontealternada}, é alimentado através de um transformador \SI{230}{\volt}/\SI{8}{\volt}, com uma frequência de \SI{50}{\hertz}.
...
\begin{figure}[hbtp]
	\centering%
		\centering
		\subfloat[\centering Rectificador de meia onda\label{fig:meiaondamenuCTRL}]{{\includegraphics[width=6cm]{figures/meiaonda_page_controlo-FULL.png} }}%
		\qquad
		\subfloat[\centering Rectificador de onda completa\label{fig:ondacompletamenuCTRL}]{{\includegraphics[width=6cm]{figures/ondacompleta_control.png} }}%
