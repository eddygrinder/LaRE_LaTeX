...
Como referido na Secção~\ref{sec:matriz}, a experiência da Lei de \textit{Ohm} requer 8 relés, enquanto as experiências de rectificadores e filtros exigem 13. Cada relé é acionado por um \textit{bit} de um registo de deslocamento, sendo que cada registo permite o controlo de até 8 relés. Assim, é necessário um registo de deslocamento para a experiência da Lei de \textit{Ohm} e dois registos para as experiências de rectificação e filtragem. Sendo assim, será enviada uma trama de 8 ou 13 \textit{bits} consoante as experiências a controlar.  

A Figura \ref{fig:SN74HC595blocos} representa o diagrama de blocos do registo e a Tabela \ref{Table:funcSN74HC595} representa os modos de funcionamento.
...
Como se pode ver na Tabela \ref{Table:funcSN74HC595}, são precisos 5 \acrshort{gpio}s configurados como saídas para controlar o envio de uma trama, portanto, para realizar o comando dos relés do \acrshort{lare}, são/serão necessários 10 \acrshort{gpio}s.
...
, correspondente a um registo de deslocamento
, correspondente a dois registos de deslocamento.
