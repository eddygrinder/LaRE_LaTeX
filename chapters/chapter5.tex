...
Como complemento aos valores reais, foi realizada a experiência com recurso ao \textit{Multisim}. Os resultados obtidos e a respectiva forma de onda estão representados na Figura \ref{fig:rippleexperimentalmeiaonda}. 

\begin{figure}[hbtp]
	\centering
	\includegraphics[width=0.7\textwidth]{figures/resultados_meia-onda.png}
	\caption{Tensão de \textit{ripple} experimental - meia onda}
	\label{fig:rippleexperimentalmeiaonda}
\end{figure}

A tensão de \textit{ripple} obtida é de, aproximadamente, \SI{1.51}{\volt}.

\textbf{NOTA PROF: Vale a pena incluir outra análise com o circuitlab?}

Os erros relativos obtidos foram de \SI{28.13}{\percent} e \SI{21.35}{\percent}, respetivamente para os valores medidos experimentalmente e simulados no Multisim. Esta diferença em relação ao valor calculado teoricamente deve-se a diversos fatores, entre os quais se destaca a simplificação inerente à expressão analítica utilizada, que assume componentes ideais e condições perfeitas — como uma resistência constante e um condensador sem perdas \cite{sedrasmith}. Por outro lado, a simulação considera comportamentos mais realistas, como a resistência interna do condensador (ESR), as quedas de tensão nos díodos e os efeitos transitórios do circuito. A maior discrepância nos valores experimentais pode ainda ser justificada por erros de leitura e pelas elevadas tolerâncias típicas dos condensadores eletrolíticos. Assim, as diferenças observadas são compreensíveis e enquadram-se dentro do esperado.
