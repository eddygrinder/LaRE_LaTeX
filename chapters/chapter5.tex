...
\begin{figure}[hbtp]
	\centering%
		...
		\qquad
		\subfloat[\centering CircuitLab\label{fig:circuitlab}]{{\includegraphics[width=6.3cm]{figures/OHM_resultado_multisim.png} }}%
		\caption{Experiência Lei de \textit{Ohm}}%
		\label{fig:experienciaOHM_simuladores}%
	\end{figure}

Através da análise do gráfico obtido no \textit{MultisimLive}, por exemplo, é possível calcular o declive da recta, dado pela Equação \ref{eq:declive}

\begin{equation} \label{eq:declive}
	\text{m} = \frac{y_{b} - y_{a}}{x_{b} - x_{a}}
\end{equation}

O valor obtido para o declive é $\text{m} \approx{1.0204}$

calcular o valor deo ero relativo e comparar com o valor real da resistência, que é de \SI{1}{\kilo\ohm}.
