...
Os resultados obtidos nesta experiência, numa fase inicial, não foram os esperados, havendo uma diferença entre os valores experimentais/simulados e teóricos com erros acima dos \SI{25}{\percent}. Por isso, foram realizados testes e medições complementares que permitiram chegar à conclusão que o valor do condensador e a tensão de saída do transformador poderiam não ser os correctos ou suficientemente estáveis para aferir os valores correctamente. De facto, entre sessões de trabalho havia flutuações na tensão de saída na ordem dos \SI{0.2}{\volt}, valores estes que  influenciam o cálculo do \textit{ripple}. Sendo assim, trocou-se o condensador por um totalmente novo, a tensão de saída do transformador foi medida várias vezes durantes os testes e todas as medições foram efectuadas na mesma sessão de trabalho. Outro aspecto importante, foram os testes efectuados no \textit{multisim}, já que a ponte recficadora utilizada no \acrshort{lare} - \textit{B250C2300-1500} \cite{B250C2306} - não está disponível neste simulador. 
